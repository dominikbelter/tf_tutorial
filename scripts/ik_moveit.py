...
def main():
    ...
    while not rospy.is_shutdown():
        # get gripper to ee_link pose
        try:
            (trans, rot) = listener.lookupTransform('/gripper', '/ee_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
        gripper_2_ee_link = pq_to_pose(trans, rot)
        # get base_link to object_link
        try:
            (trans, rot) = listener.lookupTransform('/base_link', '/object_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
        base_2_object = pq_to_pose(trans, rot)

        # transform reference position of the gripper
        ref_pose = matrix_to_pose(pose_to_matrix(base_2_object).dot(pose_to_matrix(gripper_2_ee_link)))
